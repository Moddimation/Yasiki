...
class Object:
    ...
    def resolve(self, config: "ProjectConfig", lib: Library) -> "Object":
        ...
        set_default("nolib", config.nolib)
...
class ProjectConfig:
    def __init__(self) -> None:
        ...
        self.nolib: Optional[List[str]] = (
            None  # List of non-library "libraries" to avoid archiving it
        )
...
def generate_build_ninja(
    ...
    mwld_cmd = f"{wrapper_cmd}{mwld} $ldflags -xm a -o $out @$out.rsp"
    mwld_implicit: List[Optional[Path]] = [compilers_implicit or mwld, wrapper_implicit]

    # Library
    mwld_lib_cmd = f"{wrapper_cmd}{mwld} $ldflags -xm l -o $out $in"
    ...
    #n.rule(
    #    name="ar",
    #    command=f"{dtk} ar create $out $in",
    #    description="AR $out",
    #)
    #n.newline()
    
    n.comment("Create library")
    n.rule(
        name="ar",
        command=mwld_lib_cmd,
        description="AR $out",
    )
    n.newline()
    ...
    if build_config:
        ...
        lib_files: List[Tuple[str,List[str]],str] = [] # name, objs, path

        def c_build(obj: Object, src_path: Path, build_obj: BuildConfigUnit) -> Optional[Path]:
            # Avoid double
            ...
            if config.nolib[0] != "*" and not build_obj["autogenerated"] and lib_name not in config.nolib:
                # Check if in library
                lib_entry = None
                for __lib, __file, __path in lib_files:
                    if lib_name == __lib: 
                        lib_entry = (__lib, __file)
                        break
                    
                # Add to or a library
                if lib_entry != None:
                    lib_entry[1].append(obj.src_obj_path)

                    return "EXISTING!"
                else:
                    build_dir = config.out_path()
                    lib_path = build_dir / "lib" / f"{lib_name}.a"

                    if obj.options["add_to_all"]:
                        source_inputs.append(lib_path)

                    lib_files.append( (lib_name, [obj.src_obj_path], lib_path) )
        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            if obj.src_path is not None and obj.src_path.exists():
                if file_is_c_cpp(obj.src_path):
                    ...
                    built_obj_path = c_build(obj, obj.src_path, build_obj)
        ...
        for lib,objs,path in lib_files:
            #print(lib)
            #for name in objs:
            #    print(name)
            n.build(
