...
class _Mangler:
    def __init__(self, is_remangle_mode):
        self.is_templated_function = False
        self.is_remangle_mode = is_remangle_mode


    def apply_changes(self, func, typedefs, substitutions):
    ...
    def mangle_func_ptr(self):
        ...
        if not func_name:
            mangled_func += DECORS["*"] * ptr_count+'F'

        # Else mangle the class
        else:
            mangled_func += 'M'+self.mangle_type(func_name)+'F'
        ...
        mangled_func += '_'+self.mangle_type(func_return)

        # We got it!
        return mangled_func


    def mangle_type(self, no_length):
    ...
    def isolate_func_name(self, func):
    ...
    def mangle_decorated_type(self, types):
    ...
    def mangle_operator(self, operator):
        ...
        return '__op'+self.mangle_type(operator_type)


    def mangle_function_name(self, pieces, is_const):
    ...
    def mangle_function(self, func, typedefs, substitutions):
        ...
        #    raise ValueError('Invalid function ending: '+func)
        ...
        for keyword in PREPEND_KEYWORDS:
            func_ret = func_ret.replace(keyword+' ', '').strip()
        ...
        if self.is_templated_function:
            mangled_name += '_'+self.mangle_type(func_ret)
...
def mangle_function(func):
    return _Mangler(False).mangle_function(func, None, None)
