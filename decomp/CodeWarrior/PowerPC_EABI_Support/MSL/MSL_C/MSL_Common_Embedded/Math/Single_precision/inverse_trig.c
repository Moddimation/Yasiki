...
/*
   ...
   Purpose: non-optimal single precision version of standard inverse trig
   functions Theory:  atan is a well known algorithm taken straight from
   "Computer Approximations" by Hart et. al. 1978 reprint pgs. 125-130.

*/
/* for _MIPS_ISA4/PPC we use the inverse sqrt instruction-- these are
   implemented in math_mips.h, for software fpu I've implemented _inv_sqrtf  in
   file sqrtf.c
*/
...
atanf(float x)
{
    ...
    static const float onep_one_over_xisqr_lo[] = { .000007135f, 0.00000082f, .0f, 0.00000063f, 0.0f, 0.0f };
    ...
    static const float atan_xi_lo[]
        = { 0.0f, .000009081698724f, .000000023f, .000000063f, .000000704f, .00000025f, .00000079f };

    static const float one_over_xi_hi[]
        = { 2.414213f, 1.49660575f, 1.00000000f, .668178618f, .414213568f, .198912367f };
         ...
         * (atan_coeff[1]
            + z_square
                  * (atan_coeff[2]
                     + z_square
                           * (atan_coeff[3]
                              + z_square * (atan_coeff[4] + z_square * (atan_coeff[5] + z_square * atan_coeff[6])))));
