...
#define __SQRT_FLT_EPSILON__ 1.0363331e-30f
...
// note: |z-n|<=.5 i.e. we choose n so that |n*pi/2 - x|< .5*(pi/2)=pi/4, then
// use the poly for [0,pi/4], but
//       also notice that n(mod(4)) gives which quadrant we're in and enables us
//       to use the trig addition identities. Exactly one of these terms drops
//       out because either sin(n*pi/2) or cos(n*pi/2) is 0.
#define __FAST_MATH_MODE__ 0

#if __FAST_MATH_MODE__
    _INT32      n = (_INT32)z; // note: truncation insures |n|<z and
                               // cos/sin(x)=cos/sin(n*pi/4+frac_part)
    ...
    const float frac_part
        = ((((x - (float)(n * 2)) + __four_over_pi_m1[0] * x) + __four_over_pi_m1[1] * x) + __four_over_pi_m1[2] * x)
...
__four_over_pi_m1[4]*x  */
    ...
    z = ((((__sincos_poly[1] * xsq + __sincos_poly[3]) * xsq + __sincos_poly[5]) * xsq + __sincos_poly[7]) * xsq
         + __sincos_poly[9])
        * frac_part;
