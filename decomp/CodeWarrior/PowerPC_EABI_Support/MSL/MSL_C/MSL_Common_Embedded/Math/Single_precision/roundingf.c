...
/*
 ...
 Purpose: non-optimal single precision version of standard ceil and floor
 functions Assumptions: --IEEE 754 single precision float format *fp difference
 should never produce -0 *casting a float to an int always truncates regardless
 of fp rounding mode. *the type _INT32 is 32 bits(i.e.
 sizeof(_INT32)=sizeof(float)
*/

/* ceilf--  returns smallest integer(as a float) not less than x(round x ->
   infinity) floorf-- returns largest integer(as a float) not greater than
   x(round x -> -infinity)
*/
