...
static const float __two_to_x[] = { .6931471767,    .2402265062,     .05550415684,   .009618134656, .001333182528,
...
__2_to_x(float x)
{
    ...
    float  frac_part;
    // int_part= (signbit(x) )? (_INT32)(x-.5f) : (_INT32)(x+.5f);  //won't round to even in half way cases(doesn't
    // matter)
...
expf(float x)
{
    ...
    static const float __exp_to_x[] = { .9999999009,   .4999999723,   .1666679856,    .04166688600,
                                        .008328596116, .001388275963, .0002046999276, .00002549918424 };
    if (x > 88.7228394f)
    {
        return __INFINITY;
    }
