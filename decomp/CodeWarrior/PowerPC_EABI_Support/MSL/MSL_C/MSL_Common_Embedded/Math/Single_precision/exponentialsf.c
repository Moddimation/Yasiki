 /*
   ...
   Notes:   log2f(x) is a supported part of C9X and is therefore supplied as
            a standard routine in a separate module(log2f.c).  This module is included as a header so
            that we can 
            a. inline the code and avoid the overhead of a function call.
            b. turn off checks for special values(nans,infs, etc.. )since this is already performed
               ahead of time in pow.
            

   The single precision version of expf misses less than the last 3 bits when compared with
   the built in Intel algorithm(or a max of 5 ulps which occurs only 3 out of 10,000,000 values tested).
   The result is the correctly rounded result produced from
   ...
   error analysis:
     ...
     of representable numbers(i.e. expf^^x ~= FLT_MAX, FLT_MIN).  In this case the variable transform from x to
   ...
   log2f returns the exact correctly rounded result(as is rounded from the correct IEEE double value to an
   IEEE float) > 99% of the time and misses at most the last bit.
   log2f is not a significant source of error.
   __2_to_x(float x) is basically the same routine as expf but eliminates the expensive variable transform 
   step thereby leading to better performance.  __2_to_x is meant only for the powf function and is therefore
   not exported.
   
   Reducing fraction from [-.5,.5] to [0,.5] requires an unaligned add, 2 extra multiplies, and 2 other unaligned adds
   in the reassembly step.
   
   This can be eliminated by using a similarly accurate Chebyshev polynomial on [-.5,.5].  The unaligned adds
   and additional multiplies are the majority of the error in powf which can be eliminated once this new polynomial
   is calculated.
   
   powf currently differs from the correctly rounded double result by > 2 ulps  < 10% of the time.  However, near the
   upper limits of powf's range we incur significant error(for the same reason we see this in expf).  Again the Chebyshev
   polynomial on  [-.5,.5] needs to be investigated as a means of eliminating this problem.
   
   
   Update Record
   --------------
   
   06/10/99 -- replaced original expf polynomial with a new Chebyshev polynomial converging on [-1,1] generated by Maple.
               old implementation is commented out at bottom of file.
*/
...
inline float __2_to_x(float x)
{
 ...
 z= frac_part*( __two_to_x[0] + frac_part*( __two_to_x[1] + frac_part*( __two_to_x[2] + 
    frac_part*( __two_to_x[3] + frac_part*( __two_to_x[4] + frac_part*( __two_to_x[5] +
    frac_part*( __two_to_x[6] + frac_part*( __two_to_x[7] + frac_part*__two_to_x[8])))) ))));  
...
extern "C" const float  __two_to_log2e_m1_tI[];   
 ...
 /* Maple 8th degree Chebyshev compaction polynomial for [-1,1] */
  ...
  static const float  __exp_to_x[]={.9999999009,.4999999723,.1666679856,.04166688600,
                                  .008328596116,.001388275963,.0002046999276,
                                  .00002549918424};  
  if(x > 88.7228394f) 
    return __INFINITY;
  if(x < -87.3365479f)
 ...
 *(_UINT32*)&power_of_two= (int_index) + 39 << 23;  //127-88=39;  
   ...
   asm
	...
	z=  frac_part*( __exp_to_x[0] + frac_part*( __exp_to_x[1] + frac_part*( __exp_to_x[2] + 
        frac_part*( __exp_to_x[3] + frac_part*( __exp_to_x[4] + frac_part*( __exp_to_x[5] + 
...
/* static const float  __two_to_x[]={.693147177596419,.240226604484390,
 ...
 static double log2e[]={(double)*(float*)&log2e_bits[0],(double)*(float*)&log2e_bits[1],
