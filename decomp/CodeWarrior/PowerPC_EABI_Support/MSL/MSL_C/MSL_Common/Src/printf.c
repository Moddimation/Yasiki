...
# if __option (little_endian) && __dest_os == __win32_os		/*- variables, X-86 specific -*/
	int 		offset, what_nibble = 0;						/* offset=max precision possible, what_nibble refers to high and low order nibble of working_byte */
...
/*- variables, Common-*/	
	...
	short 		*sptr;											/* sptr will point to the high order bits of the fp #, later it is masked to extract the exponent from the fp # */
...
/*- STEP6: convert FP# to ASCII char, REMEMBER you are not converting to decimal, # remains in hex form! -*/
	...
	{	
		...
		{	
			...
			/*- CONVERT TO ASCII: Two cases, Case 1 is (0 =< # < 10), Case2 is (# > 10) -*/
			if (working_byte < 10)		
