...
void* __pool_alloc_clear(__mem_pool* pool, size_t size)
{
...
void* __CALLOC(size_t nmemb, size_t size)
{
	...
	#elif __dest_os == __nec_eabi_bare || __dest_os == __emb_68k || __dest_os == __mcore_bare || __dest_os == __sh_bare
...
/*		#error */
	...
	{
		defaultheapinitialized = 1;
		#if (__dest_os == __mcore_bare) || (__dest_os == __nec_eabi_bare) || (__dest_os == __emb_68k) || (__dest_os == __sh_bare)
			if (__heap_size!= 0)
				init_alloc((void *)__heap_addr, (size_t)__heap_size);
		#else
			if (_heap_end - _heap_addr != 0)
