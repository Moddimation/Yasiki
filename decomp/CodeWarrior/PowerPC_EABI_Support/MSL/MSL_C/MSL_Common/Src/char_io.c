 ...
 * JFH 960122 Bracketed getc(), getchar(), putc(), and putchar() by #ifndef
 *__cplusplus (they are inlined in <stdio.h>). JFH 960212 Added call to
 *__stdio_atexit in __put_char for console files (so they will get flushed and closed
 *properly). JFH 960219 Added check for closed files to __get_char and __put_char.
 * JFH 960304 Allowed ungetc to work when a file is in the __neutral io_state.
 * JFH 960425 Modified __get_char, ungetc, and __ungotten to account for multi-level
 *'ungetc'. FS  970612 Added #if __dest_os == __ppc_eabi_bare for Motorola 821 SCM
 *970710 Removed __ppc_eabi_bare conditions after implementing serial console IO for
 *non-os systems. MEA 970720 Changed __no_os to _No_Disk_File_OS_Support. mm  980204
 *Changes to file structure to support wide characters mm  981029 Excluded certain
 *functions for __m56800__ US  000504 Added variation of puts() for __m56800__ mm
 *001013 Added threadsafety to fputs, puts, putc, ungetc, gets, fgets, fgetc, getc,
 *getchar
 */
