...
#else
    return (vswprintf(str, size + 1, format_str,
                      __va_start(format_str))); /*- mm 990322 -*/
#endif
}
static int
...
size_t
wcsftime(wchar_t* str, size_t max_size, const wchar_t* format_str,
    ...
    while (*format_ptr)
    {
        ...
        switch (*++format_ptr)
        {
            ...
            case L'A':
                ...
                num_chars = wemit(str, strlen(day_name[tm.tm_wday]), &max_size,
                                  L"%s", day_name[tm.tm_wday]);   /*- mm 990331 -*/
                break;
            ...
            case L'B':
                ...
                num_chars = wemit(str, strlen(month_name[tm.tm_mon]), &max_size,
                                  L"%s", month_name[tm.tm_mon]);  /*- mm 990331 -*/
                break;
            ...
            case L'I':
                num_chars = wemit(str, 2, &max_size, L"%.2d",
            ...
            case L'p':
                ...
                num_chars = wemit(str, 2, &max_size, L"%.2s",
 ...
 * mm  990322 Allow for the fact that the count wsprintf includes the terminating
 * null char.  See C9x mm  990322 Make two char string constants into wchar_t
 * constants. mm  990331 Correct length to allow for the fact that swfprintf takes %s
 * to designate a char and not wide char str mm  990809 Added include of wtime.h cc
 * 000326 Removed dest_os to be_os mm  010327 Commented wasctime and wctime out since
 * they are not defined in C99 and wasctime tampered with gmtime's threadlocal data
 * mm  010421 Corrected setting of tm_isdst cc  010510 Changed __add to __msl_add cc
 * 010510 Changed
 * __div to __msl_div cc  010510 Changed __ladd to __msl_ladd cc  010510 Changed
 * __lmul to __msl_lmul cc  010510 Changed
