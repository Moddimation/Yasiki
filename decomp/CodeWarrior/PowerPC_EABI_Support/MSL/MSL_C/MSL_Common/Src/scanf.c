			...
			case 'c':
				...
				if (arg_ptr)
				{
					...
					while (format.field_width-- && 
						(rval = (*ReadProc)(ReadProcArg, 0, __GetAChar)) != EOF)	/*- mm 990325 -*/ /*- mm 010426 -*/
					{
						c = rval;													/*- mm 010426 -*/
...
int vfscanf(FILE *  file,    const char * format, va_list arg)		/*- mm 990828 -*/
{
	int retval;														/*- mm 001013 -*/
	if (file == NULL)												/*- mm 000404 -*/
...
int vsscanf(const char * s, const char * format, va_list arg)		/*- mm 990828 -*/
{
	__InStrCtrl isc;
	isc.NextChar         = (char *)s;
 ...
 * mm  980331 Modification to allow characters >= 128, e.g. Â§ or Ã to be matched from format
