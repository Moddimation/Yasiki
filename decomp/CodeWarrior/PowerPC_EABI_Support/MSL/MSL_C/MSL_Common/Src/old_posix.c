...
int _fstat(int fildes, struct _MSL_STAT_TYPE *buf)
{
...
int _stat(const char *path, struct _MSL_STAT_TYPE *buf)
{
...
FILE *_fdopen(int fildes, const char *type)
{
...
int _read(int fildes, void *buf, size_t count)
{
...
int _write(int fildes, const void *buf, size_t count)
{
...
int _utime(const char *path, _MSL_STRUCT_UTIMEBUF_TYPE utimbuf *buf)
{
	return utime(path, buf);
}

int _utimes(const char *path, struct timeval buf[2])
{
