...
extern _INLINE float sqrtf(float x)
{
	...
	if (x > 0.0f) {
		float guess
		    = (float)__frsqrte((double)x); /* returns an approximation to	*/
		guess = _half * guess
...
extern _INLINE float _inv_sqrtf(float x)
{
	if (x > 0.0f) {
		float guess
		    = (float)__frsqrte((double)x); /* returns an approximation to	*/
		guess = .5f * guess
		        * (3.0f - guess * guess * x); /* now have 8  sig bits */
		guess = .5f * guess
		        * (3.0f - guess * guess * x); /* now have 16 sig bits */
		guess = .5f * guess
		        * (3.0f - guess * guess * x); /* now have >24 sig bits		*/
		return guess;
	} else if (x)
		return NAN;
...
extern _INLINE double fabs(double x) { return __fabs(x); }
