...
ctype<char>::ctype(const mask* tab, bool del, size_t refs)
  : locale::facet(refs), __table_(tab), __lower_map_(__lower_map), __upper_map_(__upper_map), __owns_(del)
{
    if (__table_ == 0)
    {
...
__two_exp(short x)
{
    ...
    switch (x)
    {
        case -64 :
            {
                static const _BCD one_two_to_the_negative_sixtyfourth("542101086242752217003726400434970855712890625",
                                                                      -20);
