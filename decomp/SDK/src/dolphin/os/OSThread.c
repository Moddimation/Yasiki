...
int
OSCreateThread(struct OSThread* thread, void* (*func)(void*), void* param,
...
OSSetThreadPriority(struct OSThread* thread, s32 priority)
{
    ...
    ASSERTMSGLINE(
        0x4C3, (priority >= 0) && (priority <= 0x1F),
    ...
    ASSERTMSG1LINE(0x4CA, __OSIsThreadActive(thread) != 0,
                   "OSSetThreadPriority(): thread %p is not active.", thread);
    ASSERTMSG1LINE(0x4CC, thread->state != 8,
                   "OSSetThreadPriority(): thread %p is terminated.", thread);
...
struct OSThread*
OSSetIdleFunction(void (*idleFunction)(void*), void* param, void* stack,
                  u32 stackSize)
{
    if (idleFunction)
    {
        ...
        {
            OSCreateThread(&IdleThread, (void*)idleFunction, param, stack, stackSize,
                           0x1F, 1);
...
OSCheckActiveThreads()
{
    ...
    for (prio = 0; prio <= 0x1F; prio++)
    {
        ...
        {
            ASSERTREPORT(0x566,
                         RunQueue[prio].head != NULL && RunQueue[prio].tail != NULL);
        }
        else
        {
            ASSERTREPORT(0x56B,
    ...
    ASSERTREPORT(0x572, __OSActiveThreadQueue.head == NULL ||
                            __OSActiveThreadQueue.head->linkActive.prev == NULL);
    ASSERTREPORT(0x574, __OSActiveThreadQueue.tail == NULL ||
    ...
    while (thread)
    {
        ...
        switch (thread->state)
        {
            case 1:
                if (thread->suspend <= 0)
                {
                    ASSERTREPORT(0x58C,
                                 thread->queue == &RunQueue[thread->priority]);
                    ASSERTREPORT(0x58D,
                                 IsMember(&RunQueue[thread->priority], thread));
                    ASSERTREPORT(0x58E, thread->priority ==
                                            __OSGetEffectivePriority(thread));
                }
                break;
            case 2:
            ...
            case 4:
                ...
                if (thread->suspend <= 0)
                {
                    ASSERTREPORT(0x59C, thread->priority ==
                                            __OSGetEffectivePriority(thread));
                }
                else
                {
                ...
                break;
            case 8:
                ASSERTREPORT(0x5A6, thread->queueMutex.head == NULL &&
