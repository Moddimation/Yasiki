...
OSExitThread(void *val)
{
    ...
    ASSERTMSGLINE(0x358, __OSIsThreadActive(currentThread) != 0, "OSExitThread(): current thread is not active.");
...
OSCancelThread(struct OSThread *thread)
{
    ...
    ASSERTMSG1LINE(0x37E, __OSIsThreadActive(thread) != 0, "OSExitThread(): thread %p is not active.", thread);
...
OSSleepThread(struct OSThreadQueue *queue)
{
    ...
    ASSERTMSG1LINE(0x486, __OSIsThreadActive(currentThread) != 0, "OSSleepThread(): current thread %p is not active.",
                   currentThread);
    ASSERTMSG1LINE(0x488, currentThread->state == 2, "OSSleepThread(): current thread %p is not running.",
                   currentThread);
    ASSERTMSG1LINE(0x48A, currentThread->suspend <= 0, "OSSleepThread(): current thread %p is suspended.",
                   currentThread);
...
OSSetThreadPriority(struct OSThread *thread, long priority)
{
    ...
    ASSERTMSGLINE(0x4C3, (priority >= 0) && (priority <= 0x1F),
