...
OSCreateHeap(void* start, void* end)
{
    ...
    ASSERTMSGLINE(0x2C3, (u32)ArenaStart <= (u32)start && (u32)end <= (u32)ArenaEnd,
    ...
    for (heap = 0; heap < NumHeaps; heap++)
    {
        if (HeapArray[heap].size >= 0)
        {
            ASSERTMSGLINE(0x2CF, !DLOverlap(HeapArray[heap].free, start, end),
                          "OSCreateHeap(): invalid range.");
            ASSERTMSGLINE(0x2D1, !DLOverlap(HeapArray[heap].allocated, start, end),
                          "OSCreateHeap(): invalid range.");
        }
    }
#endif
...
OSAddToHeap(int heap, void* start, void* end)
{
    ...
    ASSERTMSGLINE(0x345, (u32)ArenaStart <= (u32)start && (u32)end <= (u32)ArenaEnd,
...
OSDumpHeap(int heap)
{
    ...
    OSReport("padding %d/(%f%%) header %d/(%f%%) payload %d/(%f%%)\n",
             ...
             hd->headerBytes, (100.0 * hd->headerBytes / hd->size), hd->payloadBytes,
